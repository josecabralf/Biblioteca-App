...
class PantallaRegistrarPrestamo:
  ...
  def crearWidgets(self):
    ...
    self.widgets.append(self.crearEntryCantidadDias())
  ...
  def checkAceptar(self):
    if self.validezLibro and self.validezSocio: self.enableAceptar()
  ...
  def validarSocio(self):
    if self.campoSocio.get() == "":
      ...
      return
    try: socio = self.gestor.validarSocio(int(self.campoSocio.get()))
    except Exception as e: 
      self.mostrarError(str(e))
      return
    self.disableSocio()
    self.crearDatosSocio(socio)
    self.validezSocio = True
    self.checkAceptar()
  ...
  def aceptar(self):
    if self.campoCantidadDias.get() == "":
      self.mostrarError("Debe ingresar una cantidad de dias")
      return
    self.gestor.create(int(self.campoCantidadDias.get()))
    self.cancelar()   
  ...
  def validateInputCantDias(self, *args):
    entrada_actual = self.varCantidadDias.get()
    if entrada_actual.isdigit(): return True
    self.varCantidadDias.set(self.varCantidadDias.get()[:-1])
    return False
